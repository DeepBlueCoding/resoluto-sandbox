...
`Sandbox.run(egress=[...])` — the runtime starts a per-run SNI proxy + iptables for that run and tears
them down when it ends (no script, nothing persistent). The egress canary runs fail-closed before your
workload; there is no trusted-local bypass.
