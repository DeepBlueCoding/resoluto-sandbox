...
- **`flavor="dind"` + `privileged=True`** — docker-in-docker inside the Kata guest (privileged is
  guest-scoped under Kata, not a host escape).
- **`graph_backend="block"` + `dind_graph=...`** — put the inner docker image graph on a disk-backed
  volume instead of tmpfs, so image layers stay off RAM. `graph_backend` is ignored by non-dind steps.
- **`egress_allow` / `egress_public_https` ON THE SPEC** — per-launch egress the `k8s` runtime renders
