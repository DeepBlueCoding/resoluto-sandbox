...
Both snippets assume a `runtime` (a `SandboxRuntime` — e.g. `KataNerdctlSandboxRuntime.from_env(...)` for local or `K8sSandboxRuntime(...)` for k8s) and, for `drive_node`, a `conduit` (a `Conduit` — e.g. `store_from_env()`), wired exactly as the [`local`](https://deepbluecoding.github.io/resoluto-sandbox/0.1.0-rc1/backends/#local) / [`k8s`](https://deepbluecoding.github.io/resoluto-sandbox/0.1.0-rc1/backends/#k8s) backends build them.
