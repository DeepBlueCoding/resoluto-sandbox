...
description: Use when running a program/agent inside the resoluto-sandbox (the `Sandbox.run()` entrypoint), choosing a backend (local now; k8s on the roadmap), wiring dependency strategies (PEP723/requirements/image), the Conduit exchange abstraction, or using the Claude image with a Max subscription. Triggers — "run a script in the sandbox", "Sandbox.run", "resoluto-sandbox run", "bring my own agent", "claude image auth", "swap the backend", "conduit", "deps kind", "task.json wire".
---
...
out.exit_code   # int
out.artifacts   # dict[str, bytes] for declared output_paths
...
| kwarg | Type | Notes |
|-------|------|-------|
| `workspace` | `str \| Path` | Directory mounted (or used) as the working dir |
| `stdin` | `str \| bytes` | Passed to the program's stdin |
| `env` | `dict[str, str]` | Extra environment variables |
| `output_paths` | `list[str]` | Files to surface in `out.artifacts` |
| `stream` | `bool` | Default `True`; lines forwarded to stdout in real time |
| `deps` | `Deps` | Dependency resolution strategy (see below) |

## The two swaps

| Swap | How | Status |
|------|-----|--------|
| backend | `Sandbox(backend="local"\|"k8s")` | `local` works; `k8s` raises `NotImplementedError` (roadmap) |
| deps | `Deps(kind="auto"\|"inline"\|"requirements"\|"image"\|"vendored")` | works |
...
- **`backend="local"` is a decoupling guarantee, NOT an isolation boundary** — it runs on your host. Hardware isolation is the k8s/Kata path (roadmap).
...
- **`backend="k8s"` is not wired** — raises `NotImplementedError` today.
