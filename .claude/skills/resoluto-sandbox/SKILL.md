...
- **`workspace=None` means NOTHING is staged — it is NOT a cwd fallback.** `substrate.py`'s `if workspace:` skips `put_dir` entirely when falsy, so `/workspace` in the guest is empty and any relative argv path 404s. Always pass a real dir. When you do, argv paths are relative to that dir's CONTENTS (tarred with `arcname="."`) — `workspace="examples/payloads"` + `argv=["python","claude_agent.py",...]`, never `argv=["python","examples/payloads/claude_agent.py",...]`.
...
- **No wall-clock timeouts.** Liveness = substrate-silence (`dead_after_s=600` between chunks by default, overridable via `SubstrateBackend(dead_after_s=...)`) + heartbeat; a live program runs as long as it stays alive.

## Deep references

- `references/usage.md` — calling `run()`: full args, output collection, `result.json`, CLI.
- `references/programs.md` — bringing your own program (any language) or Claude Max-subscription auth.
