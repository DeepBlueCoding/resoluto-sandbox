...
PROVIDERS = {
    "claude":     {"payload": "claude_agent.py",    "image": "claude",
                   "auth_env": "CLAUDE_CODE_OAUTH_TOKEN", "host": "api.anthropic.com"},
    "langchain":  {"payload": "langchain_agent.py", "image": "langchain",
                   "auth_env": "ANTHROPIC_API_KEY", "host": "api.anthropic.com"},
    "openai":     {"payload": "openai_agent.py",    "image": "openai",
                   "auth_env": "OPENAI_API_KEY", "host": "api.openai.com"},
    "openrouter": {"payload": "openai_agent.py",    "image": "openai",
                   "auth_env": "OPENROUTER_API_KEY", "auth_as": "OPENAI_API_KEY", "host": "openrouter.ai",
                   "guest_env": {"OPENAI_BASE_URL": "https://openrouter.ai/api/v1",
...
def main() -> int:
    ...
    if not token:
        print(f"set {spec['auth_env']} (the {provider} provider's own credential) — the sandbox "
              f"forwards it to the guest via env=.", file=sys.stderr)
        return 1
