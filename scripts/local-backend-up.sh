...
SANDBOX_IMAGE="${RESOLUTO_SANDBOX_IMAGE:-localhost:5000/resoluto-sandbox-base:0.1.0}"
...
if ! N images "$SANDBOX_IMAGE" 2>/dev/null | grep -q resoluto-sandbox; then
  N pull --quiet --insecure-registry "$SANDBOX_IMAGE" || die "could not pull $SANDBOX_IMAGE into $NAMESPACE"
fi
