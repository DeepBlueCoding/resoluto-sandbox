#!/usr/bin/env bash
# local-backend-up.sh — bring up + verify the standalone `local` lane backend.
...
LANE_IMAGE="${RESOLUTO_LANE_IMAGE:-localhost:5000/resoluto-lane:0.1.0}"
...
# the OSS sandbox package — owns the SHARED, backend-neutral egress renderer (resoluto_sandbox.egress).
...
# scoped passwordless sudoers rule for the local-backend nerdctl binary (non-root worker)
...
step "3/7 lane bridge network ($NET_NAME $NET_SUBNET)"
...
sudo tee "$CNI_NETCONF/10-$NET_NAME.conflist" >/dev/null <<EOF
{
  ...
  "name": "$NET_NAME",
  "plugins": [
    {"type": "bridge", "bridge": "resoluto-lane0", "isGateway": true, "ipMasq": true,
...
# 4. host-side egress firewall on the lane bridge (immune to in-guest root)
step "4/7 egress firewall (default-deny; allow DNS+443-public; DROP IMDS+private)"
CHAIN="RESOLUTO-LANE-EGRESS"
# Rules come from the SHARED, backend-neutral renderer (resoluto_sandbox.egress) so the `local`
...
EGRESS_RULES="$(uv run --project "$SANDBOX_DIR" python -m resoluto_sandbox.egress local-iptables --chain "$CHAIN")"
...
pkill -f 'resoluto_sandbox.egress_proxy' 2>/dev/null || true
RESOLUTO_EGRESS_DOMAINS_FILE="$DOMAINS_FILE" setsid bash -c \
  "cd '$SANDBOX_DIR' && exec uv run python -m resoluto_sandbox.egress_proxy --host 0.0.0.0 --port $PROXY_PORT" \
...
green "ok: SNI proxy up (:$PROXY_PORT); lane :443 filtered by $DOMAINS_FILE (set per-run via Sandbox.run(egress=[...]))"

# 5. lane image present in the dedicated containerd namespace
step "5/7 lane image ($LANE_IMAGE)"
if ! N images "$LANE_IMAGE" 2>/dev/null | grep -q resoluto-lane; then
  N pull --quiet --insecure-registry "$LANE_IMAGE" || die "could not pull $LANE_IMAGE into $NAMESPACE"
fi
green "ok: lane image available"
...
RESOLUTO_LANE_IMAGE=$LANE_IMAGE
EOF
...
OUT=$(N run --rm --network "$NET_NAME" --runtime "$KATA_RUNTIME" "$LANE_IMAGE" python3 -c '
import socket
...
green "GREEN: local Kata backend up — VM isolation + egress DENY-by-default enforced (443='$(echo "$OUT" | sed -n 's/HTTPS443=//p')'). Open egress with RESOLUTO_EGRESS_ALLOW / _PUBLIC_HTTPS. RESOLUTO_LANE_BACKEND=local ready."
