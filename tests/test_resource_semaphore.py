...
async def test_no_starvation_head_reserves_against_smaller_waiters() -> None:
    ...
    async def heavy():
        await s.acquire(8 * GiB); order.append("heavy")

    async def light():
        await s.acquire(2 * GiB); order.append("light")

    th = asyncio.create_task(heavy()); await asyncio.sleep(0.02)
    tl = asyncio.create_task(light()); await asyncio.sleep(0.02)
    assert s.waiting == 2
