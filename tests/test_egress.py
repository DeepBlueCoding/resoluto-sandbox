...
def test_k8s_default_denies_public_https():
    ...
    assert not any(
        r["ports"] == [{"port": 443, "protocol": "TCP"}] for r in rules
    )  # NO public 443 by default
    assert any({"port": 53, "protocol": "UDP"} in r["ports"] for r in rules)  # DNS always


def test_k8s_public_https_true_opens_443():
    rules = k8s_egress_rules(
        EgressConfig(store_cidr="10.0.0.5/32", store_port=9100, public_https=True)
    )
...
def test_k8s_allow_adds_rule():
    rules = k8s_egress_rules(
        EgressConfig(store_cidr="10.0.0.5/32", allow=["198.51.100.0/24"], allow_port=22)
    )
...
def test_local_allow_precedes_rfc1918_reject():
    rules = _joined(
        local_egress_iptables(EgressConfig(allow=["10.1.2.3/32"], allow_port=6379), chain="EG")
    )
...
def test_local_hostname_allow_resolves(monkeypatch):
    ...
    monkeypatch.setattr(
        socket, "getaddrinfo", lambda h, *a, **k: [(2, 1, 6, "", ("203.0.113.7", 0))]
    )
...
def test_allow_hostname_resolves_into_a_rule(monkeypatch):
    ...
    monkeypatch.setattr(
        socket, "getaddrinfo", lambda h, *a, **k: [(2, 1, 6, "", ("104.18.0.7", 0))]
    )
    # a locked-down sandbox that may reach only the Anthropic API (real hostname, not a keyword)
    rules = local_egress_iptables(
        EgressConfig(allow=["api.anthropic.com"], public_https=False), chain="EG"
    )
