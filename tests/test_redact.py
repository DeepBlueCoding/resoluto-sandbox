...
def test_redact_text_scrubs_known_secret_shapes():
    ...
    assert (
        redact_text("https://x-access-token:ghp_aaaaaaaaaaaaaaaaaaaaaa@github.com/r")
        == "https://[REDACTED]@github.com/r"
    )
...
def test_redact_text_leaves_innocent_text_untouched():
    assert (
        redact_text("building module foo, 12 files compiled")
        == "building module foo, 12 files compiled"
    )
